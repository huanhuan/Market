...
- (NSMutableDictionary *)mj_keyValuesWithKeys:(NSArray *)keys subObject:(id)value{
    return [value mj_keyValuesWithKeys:nil ignoredKeys:nil];
}
    ...
    [clazz mj_enumerateProperties:^(MJProperty *property, BOOL *stop) {
        @try {
            ...
            if (!type.isFromFoundation && propertyClass) {
                value = [self mj_keyValuesWithKeys:keys subObject:value];
            } else if ([value isKindOfClass:[NSArray class]]) {
                // 3.处理数组里面有模型的情况
                if (keys) {
                    value = [[self class] mj_keyValuesArrayWithObjectArray:value keys:keys];
                }else{
                    value = [[self class] mj_keyValuesArrayWithObjectArray:value];
                }
...
- (NSData *)mj_JSONData
{
    ...
    NSError *error = nil;
    return [NSJSONSerialization dataWithJSONObject:[self mj_JSONObject] options:kNilOptions error:&error];
}
